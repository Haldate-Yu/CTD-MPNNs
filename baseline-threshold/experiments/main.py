...
from networks import Net_GCN, Net_GCNWeight, Net_GIN, Net_GIN_W, Net_SGC, Net_SSGC, Net_CTGC, GraphSAGE, \
    GraphSAGEWeight, Net_GAT, Net_Set2Set, Net_Set2SetWeight, DiffPool, Net_ASAP, Net_ASAPWeight
...
parser.add_argument('--topk', type=float, default='1.0',
                    help='topk rate of the matrix(10/20/30) / topk as a threshold')
...
print('dataset sparse: {}'.format(args.model != 'diffpool'))
dataset = get_dataset(args.dataset, sparse=args.model != 'diffpool', normalize=args.normalize, pinv=args.pinv,
                      topk=args.topk)
...
args.num_nodes_pool = dataset[0].num_nodes
...
def load_dataloader(fold_number, val_fold_number, generator):
    ...
    if 'adj' in train_set[0]:
        print('using dense loader!')
        train_loader = DenseDataLoader(dataset=train_set,
                                       batch_size=args.batch_size,
                                       shuffle=True
                                       # worker_init_fn=seed_worker,
                                       # generator=generator
                                       )
        val_loader = DenseDataLoader(dataset=val_set,
                                     batch_size=args.batch_size,
                                     shuffle=False
                                     # worker_init_fn=seed_worker,
                                     # generator=generator
                                     )
        test_loader = DenseDataLoader(dataset=test_set,
                                      batch_size=args.batch_size,
                                      shuffle=False
                                      # worker_init_fn=seed_worker,
                                      # generator=generator
...
def plot(model, loader, color=['#ffc0cb', '#bada55']):
    model.eval()
    for data in loader:
        data = data.to(args.device)
        out = model(data)
...
def model_train_and_val(dataset, seed):
    ...
    for fold_number in train_fold_iter:
        ...
        if args.model == "gcn":
            ...
            model = Net_GCNWeight(args).to(args.device)
        elif args.model == 'gat':
            model = Net_GAT(args).to(args.device)
        elif args.model == 'set2set':
            model = Net_Set2Set(args).to(args.device)
        elif args.model == 'set2set_w':
            model = Net_Set2SetWeight(args).to(args.device)
        elif args.model == 'graphsage':
            model = GraphSAGE(args).to(args.device)
        elif args.model == 'graphsage_w':
            model = GraphSAGEWeight(args).to(args.device)
        elif args.model == "gin":
            ...
            model = DiffPool(args).to(args.device)
        elif args.model == 'asap':
            model = Net_ASAP(args).to(args.device)
        elif args.model == 'asap_w':
            model = Net_ASAP(args).to(args.device)
...
def ran_exp_grid(dataset, seed_lst):
    ...
    for index, seed in enumerate(seed_lst):
        ...
        if args.model in ['gcn', 'gcn_w', 'gat', 'set2set', 'set2set_w', 'gin', 'gin_w', 'graphsage', 'graphsage_w',
                          'diffpool', 'asap', 'asap_w']:
