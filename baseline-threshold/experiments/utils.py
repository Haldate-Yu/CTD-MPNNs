...
def adj_pinv(data, dataname=None, split=0, topk=1.0):
    os.makedirs('./pinv-dataset', exist_ok=True)
    # dir for ectd-adj filter
    # os.makedirs('./pinv-dataset/test-{}'.format(dataname), exist_ok=True)
    # file = './pinv-dataset/test-{}/{}-{}.npz'.format(dataname, dataname, split)

    # dir for ectd-topk/bottomk filter
    ...
    # dir for mfpt-topk/bottomk filter,
    # it records fot mfpt-adj filter while topk=666
    # os.makedirs('./pinv-dataset/{}-topk{}'.format(dataname, topk), exist_ok=True)
    # file = './pinv-dataset/{}-topk{}/{}-{}.npz'.format(dataname, topk, dataname, split)
    ...
    if os.path.exists(file):
        ...
        # pinv = cal_pinv(adj)
        # ectd-topk/botttomk
        # pinv = cal_full_pinv(adj, topk, dataname, split)
        np.save(file, mfpt)
    ...
    # thres filter
    mfpt[mfpt < topk] = 0.
    mfpt = sp.coo_matrix(mfpt).tocoo()

    edge_index, edge_attr = from_scipy_sparse_matrix(mfpt)
...
def cal_full_pinv(adj, topk=1.0, dataname=None, split=0):
    os.makedirs('../../pinv-dataset', exist_ok=True)
    os.makedirs('../../pinv-dataset/{}-ectd-bottomk'.format(dataname), exist_ok=True)
    file = '../../pinv-dataset/{}-ectd-bottomk/{}-{}.npy'.format(dataname, dataname, split)
